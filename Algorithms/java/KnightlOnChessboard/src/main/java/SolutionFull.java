...
public class SolutionFull {
    ...
    public static void main(String[] args) {
        ...
        String  s = SolutionFull.getSolution(n);
    ...
    public  static int BFS(Cell source, Cell target,int n, int a, int b,boolean isSandBox){
        ...
        while (!queue.isEmpty()) {
            ...
            List<Cell> possibleMovements =  getPossibleMovements(node, a, b,target,visited);
            for(Cell child : possibleMovements){
                distance.put(child,nodeDistance+1);
                visited[child.getX()][child.getY()]=true;
                if(isSandBox) {
    ...
    public static long getMovements(Cell source, Cell target, int i, int j, boolean[][] visited) {
        ...
        if(source.equals(target)){
            ...
            if(!visited[source.getX()][source.getY()]){
                ...
                boolean[][] visited2 = SolutionFull.clone(visited);
    ...
    public static int[][] initDistance(int x) {
    ...
    private static class MyComparator implements Comparator<Cell> {
        ...
        public int compare(Cell o1, Cell o2) {
            long d1 = SolutionFull.getDistance(o1,target);
            long d2 = SolutionFull.getDistance(o2,target);
            if(d1>d2){
                return 1;
            }else if(d1==d2){
                return 0;
