...
public class Solution4 {
    ...
    static int getMinimumEmployees(Map<Integer,List<Integer>> myGraph,Set<Integer> burnedOut,int[] supervisors){
        Set<Integer> discovered = new HashSet<>();
        Stack<Integer> myStack = new Stack<>();
        myStack.push(0);
        int count = 0;
        Set<Integer> visitedSupervisors = new HashSet<>();
        while(!myStack.isEmpty()&&!burnedOut.isEmpty()){
            int v = myStack.pop();
            if(!discovered.contains(v)){
                ...
                if(myGraph.containsKey(v)){
                    for(Integer childs : myGraph.get(v)){
                        myStack.push(childs);
                        /*
                        if(burnedOut.contains(childs)) {
                            burnedOut.remove(childs);
                            flag = true;
                        }*/
                    }
                }else{
                    //Nodo hoja
                    if(burnedOut.contains(v)){
                        flag=true;
                        burnedOut.remove(v);
                        burnedOut.remove(supervisors[v]);
                        for(Integer sibling : myGraph.get(supervisors[v])){
